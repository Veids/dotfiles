#!/bin/bash
#Orig. Author: https://gitlab.com/vahnrr/rofi-menus

disconnect() {
    sudo nmcli con down id "$active" && notify-send "Network Manager" "Disconnected from <b>$active</b>" || notify-send "Network Manager" "Error disconnecting from <b>$active</b>!"
}
connect() {
    sudo nmcli con up id "$chosen" && notify-send "Network Manager" "Now connected to <b>$chosen</b>" || notify-send "Network Manager" "Error connecting to <b>$chosen</b>!"
}
...
mapfile -t list < <(nmcli -g name,type con | egrep '(vpn|wireguard)' | awk -F: '{print $1}')
# A vpn is active
if [ -n "$active" ]; then
    status="   Connected"
