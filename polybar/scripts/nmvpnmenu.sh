...
active="$(nmcli -g name,type con show --active 2> /dev/null | egrep '(vpn|wireguard)' | awk -F: '{print $1}')"
case $1 in
    --status)
        if [ $active ]; then
            echo " $active"
    ...
    *)
        ...
        mapfile -t list < <(nmcli -g name,type con 2> /dev/null | egrep '(vpn|wireguard)' | awk -F: '{print $1}')
        # A vpn is active
        if [ -n "$active" ]; then
            status="   Connected"
